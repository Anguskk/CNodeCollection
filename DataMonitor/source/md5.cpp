...
** 返回：无
**/
void MD5::setOriginalStr(QString str)
{
	QByteArray byteArray = str.toLatin1();
	int length = byteArray.count();
	unsigned char * inputs = (unsigned char *)byteArray.data();
	MD5Update(inputs,length);
}
...
** 返回：无
**/
void MD5::setOriginalStr(QByteArray & byteArray)
{
	int length = byteArray.count();
	unsigned char * inputs = (unsigned char *)byteArray.data();
	MD5Update(inputs,length);
...
** 返回：密文
***/
QString MD5::getEncodeStr()
{
	unsigned char digest[16];
	MD5Final(digest);
	return QString::fromLatin1((char *)digest,16);
}
