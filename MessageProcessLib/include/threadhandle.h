...
public:
    ...
    enum ThreadType
    {
        THREADSIZE, //固定线程数 网络已知，大概知道什么情况
        HANDLESIZE //固定每个线程最大处理多少连接  网络情况未知
    };

    static ThreadHandle & getClass(); //返回对象引用，是单例类  静态

    QThread * getThread(); //取出应该移入的线程
    void initThreadType(ThreadType type = HANDLESIZE,unsigned int max = 100);//初始化线程管理的方式
...
private:
    ...
    bool initfist;//状态，是否是第一次初始化，只允许初始化一次。
